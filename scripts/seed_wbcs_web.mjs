...
import { getFirestore, doc, setDoc, writeBatch, collection, query, where, getDocs } from 'firebase/firestore';
import fs from 'fs';
...
const questionsData = [
  ...
  },
  {
    ...
    correct_answer: "D",
    explanation: "Anagram of 'acme' rearranged gives 'mace'. Similarly 'alga' rearranges to 'gala'."
  },
  {
    q_no: 31,
...
async function runSeed() {
  ...
  console.log("Setting test doc in 'tests' collection...");
  await setDoc(doc(db, 'tests', testId), testMetaData, { merge: true });

  console.log("Setting PYQ doc in 'pyqs' collection...");
  await setDoc(doc(db, 'pyqs', 'pyq_wbcs_pre_2022'), pyqData, { merge: true });

  console.log("Writing questions batch...");
  const batch = writeBatch(db);
  questionsData.forEach(q => {
    const qRef = doc(db, 'questions', `${testId}_q${q.q_no}`);
    batch.set(qRef, {
      ...q,
      testId: testId,
      createdAt: Date.now()
  ...
  console.log("SUCCESSFULLY SEEDED WBCS PRELIMS 2022 TEST & PYQ RESOURCE!");
  process.exit(0);
}
