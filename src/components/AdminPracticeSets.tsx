...
import {
  ...
  Pin, RefreshCw, Eye, ExternalLink, Download, Target, Link2
} from 'lucide-react';
...
const PRACTICE_SUBJECT_OPTIONS = [
  'Mathematics Practice Set',
  'Reasoning Practice Set',
  'General Knowledge / GS',
  'English Language',
  'Computer Knowledge',
  'Full Model Practice Set (All Subjects)',
  'SSC Special Practice Set',
  'Railway Special Practice Set',
  'Banking Special Practice Set',
  'Defence & Police Practice Set',
  'General / Other'
];

export default function AdminPracticeSets() {
  const [items, setItems] = useState<PracticeSetItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [viewMode, setViewMode] = useState<'list' | 'editor'>('list');
  const [editingItem, setEditingItem] = useState<PracticeSetItem | null>(null);

  // Form states
  const [title, setTitle] = useState('');
  const [subject, setSubject] = useState(PRACTICE_SUBJECT_OPTIONS[0]);
  ...
  const fetchPracticeSets = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'practice_sets'), orderBy('createdAt', 'desc'));
      const snap = await getDocs(q);
      const list: PracticeSetItem[] = snap.docs.map(d => {
        const data = d.data();
        let legacyPdfUrl = data.pdfUrl || data.fileUrl || data.link || '';
        let legacyPdfTitle = data.pdfTitle || (legacyPdfUrl ? 'Practice Set PDF' : '');
  ...
  const handleOpenCreate = () => {
    setEditingItem(null);
    setTitle('');
    setSubject(PRACTICE_SUBJECT_OPTIONS[0]);
    setCustomSubject('');
    setFormat('pdf');
    setContent('');
    setImageUrl('');
  ...
  const handleOpenEdit = (item: PracticeSetItem) => {
    setEditingItem(item);
    setTitle(item.title || '');
    if (PRACTICE_SUBJECT_OPTIONS.includes(item.subject)) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadFileViaBackend(file, 'practice_set_images', auth.currentUser);
      setImageUrl(url);
    } catch (err: any) {
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await uploadFileViaBackend(file, 'practice_sets', auth.currentUser, file.name);
      setPdfUrl(url);
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim()) {
      alert('Please enter Practice Set Title.');
      return;
    }
    ...
    try {
      ...
      if (editingItem) {
        ...
        const docRef = doc(db, 'practice_sets', editingItem.id);
        await updateDoc(docRef, payload);
        console.log("Practice set updated successfully:", editingItem.id);
      } else {
        // CREATE new document
        payload.createdAt = serverTimestamp();
        payload.authorId = auth.currentUser?.uid || '';
        await addDoc(collection(db, 'practice_sets'), payload);
        console.log("New Practice set added successfully.");
      }

      await invalidateCacheField('practice_sets');
      await fetchPracticeSets();
      setViewMode('list');
      alert(editingItem ? 'Practice Set updated successfully!' : 'Practice Set published successfully!');
    } catch (err: any) {
      console.error("Error saving Practice Set:", err);
      alert(`Failed to save Practice Set: ${err.message}`);
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-black text-slate-800 tracking-tight">Practice Set Management</h2>
          </div>
          <p className="text-xs text-slate-500 font-medium mt-1">
            Post, edit, and update Practice Sets in Text, Image, and PDF formats for students.
          </p>
        </div>
        ...
        {viewMode === 'list' ? (
          <button
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white font-black text-xs uppercase tracking-wider rounded-2xl shadow-md transition-all flex items-center gap-2 cursor-pointer active:scale-95"
          >
        ...
        <form onSubmit={handleSave} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6 animate-in fade-in duration-200">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="text-lg font-black text-slate-800 flex items-center gap-2">
              ...
              {editingItem ? `Edit: ${editingItem.title}` : 'Add New Practice Set'}
            </h3>
            {editingItem && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 space-y-1.5">
              <label className="block text-xs font-black text-slate-500 uppercase tracking-wider">
                Practice Set Title <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
                required
                ...
                placeholder="e.g. SSC CGL 2026 Special Mathematics 50 High Yield Practice Questions"
                className="w-full rounded-2xl border-2 border-slate-200 p-3 text-sm font-bold text-slate-800 focus:border-teal-600 outline-none transition-all"
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full rounded-2xl border-2 border-slate-200 p-3 text-sm font-bold text-slate-800 focus:border-teal-600 outline-none transition-all"
              >
                {PRACTICE_SUBJECT_OPTIONS.map(opt => (
              ...
              {subject === 'General / Other' && (
                <input
                  ...
                  className="w-full rounded-2xl border-2 border-slate-200 p-3 text-sm font-bold text-slate-800 focus:border-teal-600 outline-none mt-2"
                />
              )}
            </div>
              ...
              <div className="grid grid-cols-4 gap-1.5 p-1 bg-slate-100 rounded-2xl border border-slate-200">
                <button
                  ...
                  className={`py-2 text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1 ${
                    format === 'text' ? 'bg-white text-teal-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                >
                ...
                <button
                  ...
                  className={`py-2 text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1 ${
                    format === 'image' ? 'bg-white text-teal-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                >
                ...
                <button
                  ...
                  className={`py-2 text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1 ${
                    format === 'pdf' ? 'bg-white text-teal-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                >
                ...
                <button
                  ...
                  className={`py-2 text-xs font-black rounded-xl transition-all flex items-center justify-center gap-1 ${
                    format === 'mixed' ? 'bg-white text-teal-600 shadow-xs' : 'text-slate-500 hover:text-slate-800'
                  }`}
                >
            ...
            {(format === 'text' || format === 'mixed') && (
              <div className="md:col-span-2 space-y-1.5">
                <label className="block text-xs font-black text-slate-500 uppercase tracking-wider">
                  Practice Questions Text / Solutions & Answer Key
                </label>
                <textarea
                  rows={5}
                  value={content}
                  onChange={e => setContent(e.target.value)}
                  placeholder="Enter practice questions text, options, answer keys, memory-based questions, or detailed solutions..."
                  className="w-full rounded-2xl border-2 border-slate-200 p-3 text-sm font-medium text-slate-800 focus:border-teal-600 outline-none transition-all leading-relaxed"
                />
              </div>
            )}
            ...
            {(format === 'image' || format === 'mixed') && (
              <div className="md:col-span-2 space-y-2 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                <label className="block text-xs font-black text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                  <ImgIcon className="w-4 h-4 text-teal-600" /> Question Diagram / Practice Set Screenshot
                </label>

                <div className="flex flex-wrap items-center gap-3">
                  <label className="px-4 py-2.5 bg-teal-50 hover:bg-teal-100 text-teal-700 font-extrabold text-xs rounded-xl border border-teal-200 cursor-pointer transition-all flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    className="flex-1 min-w-[200px] rounded-xl border-2 border-slate-200 p-2.5 text-xs font-medium text-slate-800 outline-none focus:border-teal-600"
                  />
                </div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border-2 border-slate-200 p-2.5 text-xs font-medium text-slate-800 outline-none focus:border-teal-600"
                />
            ...
            {(format === 'pdf' || format === 'mixed') && (
              <div className="md:col-span-2 space-y-2 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                <label className="block text-xs font-black text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                  <FileUp className="w-4 h-4 text-rose-600" /> Practice Set PDF Document
                </label>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <input
                    ...
                    placeholder="Paste Practice Set PDF URL directly"
                    className="flex-1 min-w-[200px] rounded-xl border-2 border-slate-200 p-2.5 text-xs font-medium text-slate-800 outline-none focus:border-teal-600"
                  />
                </div>
                ...
                <input
                  ...
                  placeholder="PDF Title / Download button label (e.g. Download Practice Set PDF with Solutions)"
                  className="w-full rounded-xl border-2 border-slate-200 p-2.5 text-xs font-medium text-slate-800 outline-none focus:border-teal-600"
                />
            ...
            <div className="flex items-center gap-2 pt-6">
              ...
              />
              <label htmlFor="pinned_practice" className="text-xs font-extrabold text-slate-700 cursor-pointer flex items-center gap-1">
                <Pin className="w-3.5 h-3.5 text-teal-600" /> Pin / Highlight to Top of Practice Sets
              </label>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-6 py-2.5 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-md transition-all flex items-center gap-2 disabled:opacity-50"
            >
              {saving ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
              {saving ? 'Saving...' : editingItem ? 'Update Practice Set' : 'Save & Publish Practice Set'}
            </button>
          </div>
        </form>
      )}
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
            ...
            <div className="relative flex-1 min-w-[220px]">
              ...
              <input
                type="text"
                placeholder="Search Practice Sets by title, subject or content..."
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full bg-white rounded-xl border border-slate-200 pl-9 pr-3 py-2 text-xs font-bold text-slate-800 focus:border-teal-600 outline-none"
              />
            </div>
            ...
            >
              ...
              {PRACTICE_SUBJECT_OPTIONS.map(s => (
            ...
            >
              ...
              <option value="pdf">PDF Practice Set</option>
          ...
          {loading ? (
            <div className="py-12 text-center text-slate-400 flex flex-col items-center justify-center space-y-2">
              ...
              <span className="text-xs font-medium">Loading Practice Sets...</span>
            </div>
          ) : filteredItems.length === 0 ? (
            <div className="py-12 text-center text-slate-400 font-medium text-xs">
              No Practice Set uploaded yet. Click "Add New Practice Set" to post one.
            </div>
          ) : (
              ...
              {filteredItems.map(item => (
                <div
                  ...
                  className={`p-5 rounded-3xl border transition-all flex flex-col justify-between space-y-4 relative ${
                    item.status === 'draft' ? 'bg-slate-50 border-slate-200 opacity-75' : 'bg-white border-slate-200 hover:border-teal-300 hover:shadow-md'
                  }`}
                >
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100">
                    {item.pdfUrl && (
                      ...
                      >
                        <span className="truncate flex items-center gap-1.5">
                          <FileUp className="w-3.5 h-3.5 shrink-0" /> {item.pdfTitle || 'Download Practice Set PDF'}
                        </span>
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <button
                        onClick={() => copyShareLink(item.pdfUrl || item.imageUrl || '', `practice-${item.id}`)}
                        className={`p-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 ${
                          copiedId === `practice-${item.id}` ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                        }`}
                        title="Copy share link"
                      >
                        {copiedId === `practice-${item.id}` ? <Check className="w-3.5 h-3.5" /> : <Link2 className="w-3.5 h-3.5" />}
                        {copiedId === `practice-${item.id}` ? 'Copied!' : 'Link'}
                      </button>

                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handleOpenEdit(item)}
                          className="px-3 py-1.5 bg-teal-50 text-teal-700 hover:bg-teal-100 font-extrabold text-xs rounded-xl transition-all flex items-center gap-1 cursor-pointer"
